...
def init_db():
    ...
    cur.execute("PRAGMA table_info(saved_jobs)")
    cols = [r["name"] for r in cur.fetchall()]
    if "user_email" not in cols:
        cur.execute("DROP TABLE saved_jobs")
        cur.execute("""
    ...
    app_cols = [r["name"] for r in cur.fetchall()]
...
def load_latest_profile_db(email=None):
    ...
    cur = conn.cursor()
    if email:
        cur.execute("SELECT * FROM profiles WHERE lower(email) = lower(?) ORDER BY id DESC LIMIT 1", (email.strip(),))
    else:
...
def load_saved_job_ids_db(email=None):
    conn = get_connection()
    cur = conn.cursor()
    if email:
        cur.execute("SELECT job_id FROM saved_jobs WHERE lower(user_email) = lower(?)", (email.strip(),))
    else:
        cur.execute("SELECT DISTINCT job_id FROM saved_jobs")
    ids = [r["job_id"] for r in cur.fetchall()]
    conn.close()
    return ids


def save_job_id_db(job_id, email="default"):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("INSERT OR IGNORE INTO saved_jobs (user_email, job_id) VALUES (?, ?)", (email.strip().lower(), job_id))
    conn.commit()
    conn.close()


def delete_saved_job_id_db(job_id, email="default"):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("DELETE FROM saved_jobs WHERE lower(user_email) = lower(?) AND job_id = ?", (email.strip().lower(), job_id))
    conn.commit()
    conn.close()
